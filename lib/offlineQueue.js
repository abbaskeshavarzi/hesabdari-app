...
export async function enqueueMutation({ userId, table, operation, payload, targetId = null, dependsOn = null, rpc = null }) {
...
async function replaceQueuedCustomerReferences(userId, localCustomerId, serverCustomerId) {
  ...
  await Promise.all(entries.map(async (entry) => {
    const usesLocalCustomer = entry.payload?.customer_id === localCustomerId || entry.payload?.p_customer_id === localCustomerId;
    ...
    await replaceMutation({
      ...entry,
      payload: usesLocalCustomer ? { ...entry.payload, ...(entry.payload?.p_customer_id === localCustomerId ? { p_customer_id: serverCustomerId } : { customer_id: serverCustomerId }) } : entry.payload,
...
export async function syncQueuedMutations({ userId, client }) {
  ...
  for (const entry of entries) {
    ...
    if (entry.operation === 'rpc' && entry.rpc) request = client.rpc(entry.rpc, entry.payload);
    if (!request) {
