// تبدیل دقیق تاریخ میلادی <-> شمسی (الگوریتم استاندارد جلالی)
...
// تاریخ میلادی (Date یا رشته ISO) -> [سال، ماه، روز] شمسی
export function gregorianToJalali(input) {
  const d = typeof input === 'string' ? new Date(input + 'T00:00:00') : input;
  return d2j(g2d(d.getFullYear(), d.getMonth() + 1, d.getDate()));
}

// [سال، ماه، روز] شمسی -> رشته ISO میلادی "YYYY-MM-DD"
...
// نمایش خوانا مثل "۱۴۰۵/۰۵/۱۴"
export function formatJalaliFromIso(isoDate) {
  if (!isoDate) return '';
